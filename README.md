...
**此端點刻意不做 authentication。** 本端點是設計給外部應用整合使用，而外部應用通常已具備自己的一套 authentication；若在此再加一層，等於要為了這支 API 額外維護一組獨立的帳號與憑證，徒增維運成本。

**建議由外部應用的「後端」呼叫本 API，不要直接暴露給前端。** 原因有二：

1. **資安考量**：本端點無 authentication，若由前端直接呼叫，等同於把 API 公開給任何人使用，形成資安缺口。改由後端代理呼叫，只需沿用外部應用既有的那一組 authentication 即可。
2. **串流完整性**：本端點以 SSE 串流回應，且 AI 的回覆是在**串流結束後才統一儲存**。若前端直接連線，使用者中途關閉頁面或取消請求會導致串流中斷，該次回覆將不會被儲存。由後端呼叫時，即使前端已離開，後端仍可持續接收直到串流完成，回覆才能正常寫入聊天記錄。
